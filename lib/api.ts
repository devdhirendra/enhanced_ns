...
type OnboardingRecord = {
  ...
  profileName?: string
  doc?: string[]
  processStatus: "pending" | "approved" | "rejected" | "in-progress"
...
class ApiClient {
  ...
  async createTask(
    userId: string,
    taskData: Omit<Task, "taskId" | "createdBy" | "createdAt" | "updatedAt">,
  ): Promise<ApiResponse> {
  ...
  async updateTask(
    taskId: string,
    taskData: Partial<Omit<Task, "taskId" | "createdBy" | "createdAt" | "updatedAt">>,
  ): Promise<ApiResponse> {
    return this.request(`/tasks/${taskId}`, {
      method: "PUT",
      headers: {
  ...
  async createOnboarding(data: { userId: string; doc: string[] }): Promise<ApiResponse<OnboardingRecord>> {
    return this.request<OnboardingRecord>("/onboarding/create", {
      method: "POST",
      body: JSON.stringify(data),
    })
  }
  ...
  async updateOnboardingStatus(
    id: string,
    payload: { status: "approved" | "rejected" | "pending" | "in-progress"; whoApproveUserId?: string },
  ): Promise<ApiResponse<{ message: string; updatedRecord: Partial<OnboardingRecord> }>> {
    return this.request(`/onboarding/${id}/status`, {
      method: "PATCH",
      body: JSON.stringify(payload),
    })
  }

  async deleteOnboarding(id: string): Promise<ApiResponse<{ message: string }>> {
...
export const taskApi = {
  // Task Management
  create: (userId: string, taskData: any) => apiClient.createTask(userId, taskData),

  getAll: (filters?: any) => apiClient.getAllTasks(filters),

  getAssigned: (userId: string) => apiClient.getAssignedTasks(userId),

  getCreated: (userId: string) => apiClient.getCreatedTasks(userId),

  getById: (taskId: string) => apiClient.getTask(taskId),

  update: (taskId: string, taskData: any) => apiClient.updateTask(taskId, taskData),

  delete: (taskId: string) => apiClient.deleteTask(taskId),

  assign: (taskId: string, assignTo: string) => apiClient.assignTask(taskId, assignTo),

  updateStatus: (taskId: string, status: string) => apiClient.updateTaskStatus(taskId, status),

  getStats: () => apiClient.getTaskStatistics(),

  // Logging
  getLogs: (taskId: string, filters?: any) => apiClient.getTaskLogs(taskId, filters),

  getAllLogs: (filters?: any) => apiClient.getAllSystemLogs(filters),
}
